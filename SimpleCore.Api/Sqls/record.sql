...
BEGIN
    ...
    --  id 
    CALL AddOrUpdateColumn (tbName,'Id','char(16) NOT NULL COMMENT ''系统唯一编号''','PRIMARY KEY');
    --  createTime 
    CALL AddOrUpdateColumn (tbName,'CreateTime','datetime NOT NULL COMMENT ''新建该账号的时间''','');
    --  createUserId 
    CALL AddOrUpdateColumn (tbName,'CreateByUserId','char(64) NOT NULL COMMENT ''新建者Id''','');
    --  createUserName 
    CALL AddOrUpdateColumn (tbName,'CreateByUserName','varchar(80) NOT NULL COMMENT ''新建者名称''','');
    --  modifyTime 
    CALL AddOrUpdateColumn (tbName,'UpdateTime','datetime NOT NULL COMMENT ''修改该账号的时间''','');
    --  modifyUserId 
    CALL AddOrUpdateColumn (tbName,'UpdateByUserId','char(64) NOT NULL COMMENT ''新建者Id''','');
    --  modifyUserName 
    CALL AddOrUpdateColumn (tbName,'UpdateByUserName','varchar(80) NOT NULL COMMENT ''修改者名称''','');

END$$
DELIMITER ;


CALL AddOrUpdateRecordTable('Account');